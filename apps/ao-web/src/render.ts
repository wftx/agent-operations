...
export function renderApprovals(actions: readonly DurableRepositoryRestoreAction[]): string {
  ...
  const content = pending.length ? pending.map(action => `<article class="card">
    ...
    <div class="actions"><a class="button secondary" href="/repository-restores/${encodeURIComponent(action.id)}">View action</a><form method="post" action="/repository-restores/${encodeURIComponent(action.id)}/approve"><input type="hidden" name="approvedBy" value="human-operator"><button class="danger">Approve Exact Restore</button></form></div>
  ...
  const history = actions.filter(action => action.state !== 'pending-approval').reverse().map(action => `<article class="card"><div class="card-head"><div><span class="status ${action.state === 'completed' ? 'done' : 'needs'}">${escapeHtml(humanize(action.state))}</span><h2>Selective tracked file restore</h2></div><time>${formatTime(action.result?.completedAt ?? action.approvedAt ?? action.createdAt)}</time></div><p>${escapeHtml(action.approvalSummary)}</p><div class="actions"><a class="button secondary" href="/repository-restores/${encodeURIComponent(action.id)}">View action and receipt</a></div></article>`).join('');
  return layout('Approvals', `<header class="page-head"><div><p class="eyebrow">Human authority</p><h1>Approvals</h1><p>Destructive operations remain bound to exact repository preconditions and immutable execution receipts.</p></div><a class="refresh" href="/approvals">Refresh</a></header><section class="stack">${content}${history}</section>`, 'approvals');
}

export function renderRepositoryRestoreDetail(
  action: DurableRepositoryRestoreAction,
  receipt: RepositoryRestoreExecutionReceipt | null,
): string {
  const hashes = receipt ? [...receipt.restoredFileHashesAfter, ...receipt.preservedUntrackedFileHashesAfter]
    .map(item => `<div><dt>${escapeHtml(item.path)}</dt><dd>${escapeHtml(item.sha256)}</dd></div>`).join('') : '';
  const content = `<a class="back" href="/approvals">← Approvals</a><header class="detail-head"><div><span class="status ${action.state === 'completed' ? 'done' : 'needs'}">${escapeHtml(humanize(action.state))}</span><h1>Selective tracked file restore</h1><p>${escapeHtml(action.id)}</p></div><time>${formatTime(action.createdAt)}</time></header>
    <section class="criteria"><p class="eyebrow">Approved Red action</p><p>${escapeHtml(action.approvalSummary)}</p><dl class="facts"><div><dt>Project</dt><dd>${escapeHtml(action.projectId)}</dd></div><div><dt>Repository</dt><dd>${escapeHtml(action.repositoryId)}</dd></div><div><dt>Checkout binding</dt><dd>${escapeHtml(action.checkoutBindingId)}</dd></div><div><dt>Approved paths</dt><dd>${escapeHtml(action.preconditions.restorePaths.map(item => item.path).join('\n'))}</dd></div><div><dt>Approval</dt><dd>${escapeHtml(action.approvedBy ?? 'pending')} · ${action.approvedAt ? formatTime(action.approvedAt) : 'pending'}</dd></div></dl></section>
    ${receipt ? `<section class="criteria"><p class="eyebrow">Immutable execution receipt</p><p><strong>Verification ${escapeHtml(receipt.verificationResult)}</strong> · executed ${formatTime(receipt.executedAt)} · recorded ${formatTime(receipt.recordedAt)}</p><dl class="facts"><div><dt>Branch</dt><dd>${escapeHtml(receipt.branchBefore)} → ${escapeHtml(receipt.branchAfter)}</dd></div><div><dt>HEAD</dt><dd>${escapeHtml(receipt.headBefore)} → ${escapeHtml(receipt.headAfter)}</dd></div><div><dt>Commit</dt><dd>${receipt.commitPerformed ? 'yes' : 'no'}</dd></div><div><dt>Push</dt><dd>${receipt.pushPerformed ? 'yes' : 'no'}</dd></div><div><dt>Deploy</dt><dd>${receipt.deployPerformed ? 'yes' : 'no'}</dd></div><div><dt>Final status</dt><dd>${escapeHtml(receipt.finalStatus.map(item => `${item.path}: ${item.indexStatus}/${item.worktreeStatus}`).join('\n') || 'clean')}</dd></div>${hashes}</dl></section>` : `<div class="alert error"><strong>Execution receipt unavailable.</strong><br>This action predates receipt capture or has not executed.</div>`}`;
  return layout('Red action detail', content, 'approvals');
}
